...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func shareWithVkontakte(){
        println("Perform photo upload with VK")
        let Config = VkConfig(
            clientId: "4533498",
            clientSecret: "jTPRfVf83NGJZyITVGm5",
            scopes:["docs","friends"])
        
        let fbModule =  AccountManager.addVKAccount(Config)
        self.http.authzModule = fbModule
        fbModule.login { (AnyObject, OpenIDClaim, NSError) -> Void in
